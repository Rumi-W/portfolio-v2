...
import { Card, CardContent, Grid, Typography, Fade } from '@material-ui/core'
...
const SkillCard = ({ title, content, links, linksRepo }) => {
  ...
  const linksSection = links.map((link, i) => (
    <CustomLink key={link.id} linkInfo={link} type="link" />
  ))
  const repoLinksSection = linksRepo.map((link, i) => (
    <CustomLink key={link.id} linkInfo={link} type="repo" />
  ))
